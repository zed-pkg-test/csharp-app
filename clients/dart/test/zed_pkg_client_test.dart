...
void main() {
  ...
  test('insecure download urls are rejected', () {
    ...
    for (final raw in ['http://evil.example/a', 'file:///etc/passwd']) {
      expect(() => client.allowedDownloadUrl(raw, base),
          throwsA(isA<ZedApiError>()));
    }
    expect(client.allowedDownloadUrl('https://cdn.example/a', base),
        'https://cdn.example/a');
    expect(client.allowedDownloadUrl('http://127.0.0.1:8080/a', base),
  ...
  test('downloadArtifact verifies sha256 and omits the token', () async {
    ...
    final zed = ZedClient(httpClient: mock, token: 'zpkg_t');
    final bytes = await zed
        .downloadArtifact(makeVersion(sha256: digest, size: body.length));
    expect(bytes, body);
  ...
  test('downloadArtifact rejects an oversize body', () async {
    ...
    final mock = MockClient((request) async =>
        http.Response.bytes(List.filled(limit + 64, 0), 200));
    final zed = ZedClient(httpClient: mock);
    expect(
      () => zed.downloadArtifact(makeVersion(sha256: 'deadbeef', size: 1)),
      throwsA(isA<ZedApiError>()
          .having((e) => e.code, 'code', 'artifact_too_large')),
    );
  });
