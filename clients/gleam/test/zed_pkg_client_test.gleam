...
pub fn insecure_download_urls_are_rejected_test() {
  ...
  |> should.be_error
  client.allowed_download_url("https://cdn.example/a", base)
  |> should.equal(Ok("https://cdn.example/a"))
...
pub fn non_json_error_bodies_map_to_unknown_test() {
  let zed =
    client.new("https://registry.zpkg.tech")
...
pub fn yank_posts_the_flag_test() {
  let body =
    "{\"org\":\"acme\",\"name\":\"kit\",\"version\":\"1.2.0\",\"yanked\":true}"
  let zed =
    client.new("https://registry.zpkg.tech")
    |> client.with_token("zpkg_t")
    |> client.with_transport(
...
pub fn download_verifies_sha_and_omits_auth_test() {
  ...
  let zed =
    ...
    |> client.with_transport(fn(req, _timeout) {
      ...
      |> should.be_error
      req.path
      |> should.equal("/v1/artifacts/" <> digest)
      Ok(response.Response(status: 200, headers: [], body: bytes))
    })
  client.download_artifact(
    zed,
    make_version(sha256: digest, size: 14, download_url: ""),
  )
...
pub fn download_rejects_oversize_body_test() {
  ...
  |> should.equal(Error(client.ArtifactTooLarge(limit: limit)))
}
...
pub fn multipart_body_layout_test() {
  ...
  |> should.be_true
  string.contains(text, "name=\"artifact\"; filename=\"acme-kit-1.2.0.tar.gz\"")
  |> should.be_true
  string.ends_with(text, "--BOUNDARY--\r\n")
  |> should.be_true
}

pub fn publish_sends_multipart_put_test() {
  let body =
